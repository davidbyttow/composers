 ...
 * @return {!Node}
 */
Node.constant = function (outputKey, value) {
...
Graph.prototype.start = function () {
  ...
  return this._resolveGraph(node)
}
...
Graph.prototype._getNode = function (key) {
  if (this._explicitInputs.has(key)) {
    return Node.constant(key, this._explicitInputs.get(key))
  }

  // TODO(david): Error out if overriding an already implicit nodes.
...
Scope.prototype.getNode = function (outputKey) {
  if (this._seedCache.has(outputKey)) {
    return Node.constant(outputKey, this._seedCache.get(outputKey))
  }
